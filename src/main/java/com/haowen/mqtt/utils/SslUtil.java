...
public class SslUtil {
	public static SSLSocketFactory getSocketFactory(final String caCrtFile, final String crtFile, final String keyFile,
			final String password,ByteReader byteReader) throws Exception {
		Security.addProvider(new BouncyCastleProvider());

		// load CA certificate
		PEMReader reader = new PEMReader(
				new InputStreamReader(new ByteArrayInputStream(byteReader.readAsByte(caCrtFile))));
		...
		reader = new PEMReader(new InputStreamReader(new ByteArrayInputStream(byteReader.readAsByte(crtFile))));
		X509Certificate cert = (X509Certificate) reader.readObject();
		reader.close();

		// load client private key
		reader = new PEMReader(new InputStreamReader(new ByteArrayInputStream(byteReader.readAsByte(keyFile))),
				new PasswordFinder() {
					@Override
